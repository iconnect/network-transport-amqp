  ...
  try . asyncWhenCancelled closeEndPoint $ do
    ...
    return EndPoint
      ...
      , closeEndPoint = let evs = [ EndPointClosed
                        ...
                        apiCloseEndPoint is ourLocalEp evs ourEndPoint
...
apiCloseEndPoint :: AMQPInternalState 
                 ...
                 -> T.Text
                 -> IO ()
apiCloseEndPoint AMQPInternalState{..} lep evts ourEp = do
  ...
  modifyValidLocalState lep $ \vst@ValidLocalEndPointState{..} -> do
    ...
    let queue = fromAddress $ localAddress lep
...
apiCloseTransport is =
  modifyMVar_ (istate_tstate is) $ \tst -> case tst of
    ...
    TransportValid (ValidTransportState cnn mp) -> do
      mapM_ (\ep -> modifyMVar_ (localState ep) $ const $ return LocalEndPointClosed) (Map.elems mp)
