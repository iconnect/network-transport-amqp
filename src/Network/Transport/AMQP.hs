...
data AMQPContext = AMQPContext {
  ...
  , ctx_endpoint :: !T.Text -- ^ The queue and exchange name
...
apiNewEndPoint tr@AMQPTransport{..} = do
  try . asyncWhenCancelled closeEndPoint $ do
    ...
    (ourEndPoint,_,_) <- AMQP.declareQueue transportChannel $ AMQP.newQueue {
                           AMQP.queueName = toS . toString $ uid
    ...
    return EndPoint
      { receive       = readChan (ctx_evtChan ctx)
...
apiCloseEndPoint AMQPTransport{..} AMQPContext{..} _ _ = do
...
apiConnect tr ctx@AMQPContext{..} theirAddress _ _ = do
  try . asyncWhenCancelled close $ do
    if (EndPointAddress . toS $ ctx_endpoint) == theirAddress
...
apiSend AMQPTransport{..} AMQPContext{..} their connId msgs = Right <$>
    AMQP.publishMsg transportChannel
                    (toS . endPointAddressToByteString $ their)
                    ctx_endpoint
                    (AMQP.newMsg { AMQP.msgBody = encode' $ MessageData connId msgs
